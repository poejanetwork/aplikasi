(function ($) {
    ...
    function loadData(page = 1) {
        currentPage = page;
        $.get(APP.getUrl + '/user_group/data', {
            page: page,
    ...
    $(document).on('submit', '#formAddData', function (e) {
        ...
        let form = $(this);
        $.ajax({
            url: APP.getUrl + '/user_group/store',
            type: 'POST',
            data: form.serialize(),
            dataType: 'json',
    ...
    $(document).on('submit', '#formEditData', function (e) {
        ...
        $.ajax({
            url: APP.getUrl + '/user_group/update/' + id,
            type: 'POST',
    ...
    window.deleteData = function (id) {
        swalDelete('Hapus data?','Data tidak bisa dikembalikan').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: APP.getUrl + '/user_group/delete/' + id,
                    type: 'POST',
                    data: { _method: 'DELETE' },
                    dataType: 'json',
                    success: function (res) {
    ...
    window.restoreData = function (id) {
        swalDelete('Restore data?','Data akan dikembalikan').then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: APP.getUrl + '/user_group/restore/' + id,
                    type: 'POST',
                    data: { _method: 'PATCH' },
                    dataType: 'json',
                    success: function (res) {

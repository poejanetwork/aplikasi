     ...
     <div class="row mx-0">
		   ...
		   <div class="col-md-10 p-0">
		     <div class="contact_righto  p-3 bg-white">
			      <span class="mb-0 font_11 light_gray"><a class="fw-bold" href="{$settings.BASE_URL}">Home</a> <span class="mx-2">/</span> News Detail</span>
			 </div>
		   </div>
		 </div>
   </div>
	  ...
	  <div class="col-md-8">
	     <div class="news_dt">
		    <div class="news_dt1 text-center">
			    <a href="javascript:void(0);"><img  class="img-fluid" alt="{$data.title}" src="{$data.thumbnail}"></a>
			</div>
			<div class="news_dt2 p-4 bg-white">
			      <b class="d-inline-block bg_yellow text-white p-1 px-3 font_10 text-uppercase rounded-1">{$data.category}</b>
					 <b class="d-block fs-3 text-uppercase mt-2 mb-1"><a href="javascript:void(0);">{$data.title}</a></b>
				    <ul class="mb-0 font_11 fw-bold text-uppercase justify-content-between d-flex">
		   <li>
		   <a href="javascript:void(0);"><img class="rounded-circle" alt="{$data.username} Avatar" src="{$theme}/assets/image/9.png"></a>
		   <span class="gray_dark ms-2">{$data.username}</span>
		   </li>
		   <li class="my-auto">
			 <a class="gray_dark mx-3" href="javascript:void(0);" title="Tanggal posting"><i class="fa fa-calendar me-1 text-success"></i> {$data.created_at}</a>
			 <a class="gray_dark" href="javascript:void(0);" title="Total dibaca"><i class="fa fa-eye me-1 text-info"></i> {$data.views_count}</a>
		   </li>
		 </ul>
			<p class="mt-4 text-dark fs-6 text-justify">{$data.full_text_bbcode}</p>
			<ul class="mb-0 d-flex flex-wrap text-uppercase font_11 tags  mt-4">
				
			{foreach $data.tags as $tag => $weight}
				<li class="mx-1 mt-1 mb-1"><a class="d-block border p-1 px-2 tag-size-{$weight}" href='{"news/tag/search?q={$tag}"|surl}'>
					{$tag}
				</a></li>
			{/foreach}
		   </ul>
